...
from ..schemas import (
    ConfluenceAuthUrlOut,
    ConfluenceCredentialStatusOut,
    ConfluenceCredentialUpsertRequest,
    ConfluenceIntegrationOut,
    ConfluenceSettingsUpdateRequest,
    ConfluenceSourcesUpdateRequest,
    ConfluenceSpaceOut,
    ConfluenceSyncJobOut,
)
from ..security_utils import encrypt_secret
from ..services.connectors import confluence as confluence_service
...
router = APIRouter(prefix="/api/v1/connectors/confluence", tags=["Confluence Connector"])
_REDIRECT_SUFFIX = "/connectors/confluence/auth/callback"
...
def _resolve_credentials(db: Session, user_id: int, project_id: uuid.UUID) -> tuple[str, str, str]:
    existing = (
        ...
        .filter(
            ...
            ConnectorProjectCredential.connector_type == CONNECTOR_TYPE_CONFLUENCE,
        )
        .first()
    )
    if not existing:
        raise HTTPException(status_code=400, detail="Confluence OAuth credentials are not configured for this project")
    from ..security_utils import decrypt_secret

    return existing.client_id, decrypt_secret(existing.client_secret_encrypted), existing.redirect_uri


def _integration_out(integration: ConnectorIntegration) -> ConfluenceIntegrationOut:
    sources = integration.sources.sources if integration.sources else {}
    return ConfluenceIntegrationOut(
        id=integration.id,
        ...
        settings=validate_confluence_settings(integration.settings.settings if integration.settings else {}),
        sources={"spaces": sources.get("spaces") or [], "pages": sources.get("pages") or []},
        created_at=integration.created_at,
        updated_at=integration.updated_at,
    )


@router.get("/auth/start", response_model=ConfluenceAuthUrlOut)
def auth_start(
    ...
    return ConfluenceAuthUrlOut(
        auth_url=confluence_service.get_auth_url(str(project_id), current_user.id, client_id, redirect_uri)
    )
...
def auth_callback(
    ...
    state_data = confluence_service.parse_oauth_state(state)
    ...
    tokens = confluence_service.exchange_code_for_tokens(code, redirect_uri, client_id, client_secret)
    label = tokens.get("site_name") or "Confluence"
    ...
    if existing:
        ...
        existing.drive_page_token = tokens.get("cloud_id")
        ...
        integration = ConnectorIntegration(
            ...
            is_active=True,
            drive_page_token=tokens.get("cloud_id"),
        )
        ...
        sources_row.sources = {"spaces": [], "pages": []}
    db.commit()

    app_origin = (settings.frontend_base_url or "").rstrip("/")
    post_message = ""
    if app_origin:
        post_message = f"window.opener.postMessage({{ type: 'connector_connected', connector: 'confluence' }}, '{app_origin}');"
    return HTMLResponse(
        content=f"""<!DOCTYPE html><html><body><p>Confluence connected! Closing...</p><script>if (window.opener) {{{post_message}}} window.close();</script></body></html>"""
    )


@router.post("/credentials", response_model=ConfluenceCredentialStatusOut)
def upsert_credentials(
    payload: ConfluenceCredentialUpsertRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ...
    db.commit()
    return ConfluenceCredentialStatusOut(
        configured=True,
        client_id=payload.client_id,
        redirect_uri=payload.redirect_uri,
...
@router.get("/credentials/status", response_model=ConfluenceCredentialStatusOut)
def credentials_status(
    ...
    if not try_connector_project_access(db, current_user, project_id):
        return ConfluenceCredentialStatusOut(configured=False)
    existing = (
        db.query(ConnectorProjectCredential)
        .filter(
            ConnectorProjectCredential.user_id == current_user.id,
...
@router.get("/status", response_model=Optional[ConfluenceIntegrationOut])
def integration_status(
    ...
    integration = get_integration(
        db, user_id=current_user.id, project_id=project_id, connector_type=CONNECTOR_TYPE_CONFLUENCE
    )
    if not integration:
        return None
...
@router.get("/spaces", response_model=List[ConfluenceSpaceOut])
def list_spaces(
    project_id: uuid.UUID = Query(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
    ...
    if not integration:
        raise HTTPException(status_code=404, detail="Confluence is not connected")
    return [ConfluenceSpaceOut(id=x["id"], key=x.get("key"), name=x["name"]) for x in confluence_service.list_spaces(db, integration)]


@router.post("/sources", response_model=ConfluenceIntegrationOut)
def update_sources(
    payload: ConfluenceSourcesUpdateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ensure_connector_project_access(db, current_user, payload.project_id)
    integration = get_integration(
        db, user_id=current_user.id, project_id=payload.project_id, connector_type=CONNECTOR_TYPE_CONFLUENCE
    )
    if not integration:
        raise HTTPException(status_code=404, detail="Confluence is not connected")
    sources_row = get_or_create_sources_row(db, integration.id)
    sources_row.sources = {"spaces": [x.model_dump() for x in payload.spaces], "pages": [x.model_dump() for x in payload.pages]}
    db.commit()
    db.refresh(integration)
    return _integration_out(integration)


@router.post("/settings", response_model=ConfluenceIntegrationOut)
def update_settings(
    payload: ConfluenceSettingsUpdateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ensure_connector_project_access(db, current_user, payload.project_id)
    ...
    settings_row.settings = validate_confluence_settings(payload.settings)
    db.commit()
    db.refresh(integration)
    return _integration_out(integration)


@router.post("/sync", response_model=ConfluenceSyncJobOut)
def trigger_sync(
    project_id: uuid.UUID = Query(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
    ...
    if not enqueue_connector_sync(db, integration=integration, sync_job_id=sync_job.id):
        confluence_service.run_confluence_sync(db, str(integration.id), str(sync_job.id))
    db.refresh(sync_job)
    return ConfluenceSyncJobOut(
        id=sync_job.id,
...
@router.get("/jobs", response_model=List[ConfluenceSyncJobOut])
def list_jobs(
    project_id: uuid.UUID = Query(...),
...
@router.post("/pause", response_model=ConfluenceIntegrationOut)
...
@router.post("/resume", response_model=ConfluenceIntegrationOut)
...
def disconnect(
    ...
    db.commit()
    return {"message": "Confluence disconnected"}
