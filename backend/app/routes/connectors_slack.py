...
from ..schemas import (
    SlackAuthUrlOut,
    SlackChannelOut,
    SlackCredentialStatusOut,
    SlackCredentialUpsertRequest,
    SlackIntegrationOut,
    SlackSettingsUpdateRequest,
    SlackSourcesUpdateRequest,
    SlackSyncJobOut,
)
from ..security_utils import decrypt_secret, encrypt_secret
from ..services.connectors import slack as slack_service
...
router = APIRouter(prefix="/api/v1/connectors/slack", tags=["Slack Connector"])
_REDIRECT_SUFFIX = "/connectors/slack/auth/callback"
...
def _resolve_credentials(db: Session, user_id: int, project_id: uuid.UUID) -> tuple[str, str, str]:
    row = (
        ...
        .filter(
            ...
            ConnectorProjectCredential.connector_type == CONNECTOR_TYPE_SLACK,
        )
        .first()
    )
    if not row:
        raise HTTPException(status_code=400, detail="Slack OAuth credentials are not configured for this project")
    return row.client_id, decrypt_secret(row.client_secret_encrypted), row.redirect_uri


def _integration_out(integration: ConnectorIntegration) -> SlackIntegrationOut:
    ...
    return SlackIntegrationOut(
        ...
        settings=validate_slack_settings(integration.settings.settings if integration.settings else {}),
        sources={"channels": sources.get("channels") or []},
        created_at=integration.created_at,
        updated_at=integration.updated_at,
    )


@router.get("/auth/start", response_model=SlackAuthUrlOut)
def auth_start(
    ...
    return SlackAuthUrlOut(auth_url=slack_service.get_auth_url(str(project_id), current_user.id, client_id, redirect_uri))
...
def auth_callback(
    ...
    state_data = slack_service.parse_oauth_state(state)
    ...
    tokens = slack_service.exchange_code_for_tokens(code, redirect_uri, client_id, client_secret)
    label = tokens.get("team_name") or "Slack workspace"
    existing = get_integration(
        db, user_id=int(user_id), project_id=uuid.UUID(project_id), connector_type=CONNECTOR_TYPE_SLACK
    )
    if existing:
        ...
        sources_row.sources = {"channels": []}
    db.commit()
    app_origin = (settings.frontend_base_url or "").rstrip("/")
    post_message = ""
    if app_origin:
        post_message = f"window.opener.postMessage({{ type: 'connector_connected', connector: 'slack' }}, '{app_origin}');"
    return HTMLResponse(content=f"<html><body><p>Slack connected! Closing...</p><script>if (window.opener) {{{post_message}}} window.close();</script></body></html>")


@router.post("/credentials", response_model=SlackCredentialStatusOut)
def upsert_credentials(
    payload: SlackCredentialUpsertRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ...
    db.commit()
    return SlackCredentialStatusOut(configured=True, client_id=payload.client_id, redirect_uri=payload.redirect_uri)


@router.get("/credentials/status", response_model=SlackCredentialStatusOut)
def credentials_status(
    ...
    if not try_connector_project_access(db, current_user, project_id):
        return SlackCredentialStatusOut(configured=False)
    row = (
        db.query(ConnectorProjectCredential)
        .filter(
            ConnectorProjectCredential.user_id == current_user.id,
    ...
    if not row:
        return SlackCredentialStatusOut(configured=False)
    return SlackCredentialStatusOut(
        configured=True,
...
@router.get("/status", response_model=Optional[SlackIntegrationOut])
def integration_status(
    ...
    integration = get_integration(
        db, user_id=current_user.id, project_id=project_id, connector_type=CONNECTOR_TYPE_SLACK
    )
...
def list_channels(
    ...
    return [SlackChannelOut(**x) for x in slack_service.list_channels(token)]


@router.post("/sources", response_model=SlackIntegrationOut)
def update_sources(
    payload: SlackSourcesUpdateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ensure_connector_project_access(db, current_user, payload.project_id)
    integration = get_integration(
        db, user_id=current_user.id, project_id=payload.project_id, connector_type=CONNECTOR_TYPE_SLACK
    )
    if not integration:
        raise HTTPException(status_code=404, detail="Slack is not connected")
    row = get_or_create_sources_row(db, integration.id)
    row.sources = {"channels": [x.model_dump() for x in payload.channels]}
    db.commit()
    db.refresh(integration)
    return _integration_out(integration)


@router.post("/settings", response_model=SlackIntegrationOut)
def update_settings(
    payload: SlackSettingsUpdateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
):
    ensure_connector_project_access(db, current_user, payload.project_id)
    ...
    row.settings = validate_slack_settings(payload.settings)
    db.commit()
    db.refresh(integration)
    return _integration_out(integration)


@router.post("/sync", response_model=SlackSyncJobOut)
def trigger_sync(
    project_id: uuid.UUID = Query(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_required),
    ...
    if not enqueue_connector_sync(db, integration=integration, sync_job_id=sync_job.id):
        slack_service.run_slack_sync(db, str(integration.id), str(sync_job.id))
    db.refresh(sync_job)
    return SlackSyncJobOut(
        id=sync_job.id,
...
@router.get("/jobs", response_model=List[SlackSyncJobOut])
def list_jobs(
    project_id: uuid.UUID = Query(...),
...
@router.post("/pause", response_model=SlackIntegrationOut)
...
@router.post("/resume", response_model=SlackIntegrationOut)
