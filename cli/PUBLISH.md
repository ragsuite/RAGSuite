...
Current: **`1.0.6`** — same value in:

| Place | Value |
|------|--------|
| npm | `@ragsuite/ragsuite@1.0.6` (publish separately via gated workflow) |
| `cli/package.json` | `"version": "1.0.6"` |
| Platform | `PLATFORM_VERSION` in `backend/app/platform/version.py` |
| GitHub tag | **`v1.0.6`** (created automatically on push to `main` if missing) |
| EE | `RAGSUITE_EE/VERSION` → `ee-v1.0.6` / `ragsuite-ee-1.0.6.tar.gz` |
...
```bash
npm install -g @ragsuite/ragsuite@1.0.6
ragsuite version
ragsuite init
ragsuite start
