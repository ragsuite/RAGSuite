...
import { handleTestAiAssistantSettings } from '@/network/actions/ai-assistant.actions';
import {
...
export type AiAssistantBeforeConnectionTestResult = {
  abort?: boolean;
...
type Props = {
  ...
  onBeforeTest?: () => Promise<AiAssistantBeforeConnectionTestResult | void>;
  onTestComplete?: () => void;
};

export function AiAssistantApiKeyConnectionHint({
  projectId,
  provider,
  apiKey,
  chatModel,
  ...
  useEffect(() => {
    setStatus('idle');
    setMessage('');
  }, [provider, apiKey, chatModel, baseUrl]);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const pending = before.clearedPending ? null : pendingPlaintextApiKey;
      const keyForTest =
        pending && pending.trim() && !isMaskedApiKey(pending) ? pending.trim() : undefined;

      const result = await handleTestAiAssistantSettings(projectId, {
      ...
      <View style={[styles.row, { gap: spacing.sm }]}>
        {showSavedBadge ? (
          <View style={[styles.row, { gap: 4 }]}>
            ...
            <Text style={[typography.caption, { color: successColor }]}>
              {t('models.apiKey.savedHint')}
            </Text>
          </View>
        ) : null}
        <Pressable
