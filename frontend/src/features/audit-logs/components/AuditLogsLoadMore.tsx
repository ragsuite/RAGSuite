...
export function AuditLogsLoadMore({ loadedCount, total, hasMore, loadingMore, onLoadMore }: Props) {
  ...
  return (
    <View style={[styles.wrap, { marginTop: spacing.md, gap: spacing.sm, paddingBottom: spacing.xs }]}>
      <Text style={[typography.caption, { color: colors.textMuted, textAlign: 'center', fontWeight: '500' }]}>
        Showing {loadedCount} of {total} events
      </Text>
      ...
      {hasMore ? (
        <Pressable
          ...
          {loadingMore ? (
            ...
            <Text style={[typography.body, { color: colors.text, fontWeight: '500' }]}>{t('audit.loadMore')}</Text>
          )}
        </Pressable>
      ) : null}
    </View>
