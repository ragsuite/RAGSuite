...
import { getTeamsOAuthRedirectUri } from '@/features/crawl/utils/teams-oauth';
import {
  handleDisconnectTeams,
  handleGetTeamsAuthUrl,
  handleGetTeamsCredentialStatus,
  handleGetTeamsJobs,
  handleGetTeamsStatus,
  handlePauseTeams,
  handleResumeTeams,
  handleSaveTeamsSettings,
  handleSaveTeamsSources,
  handleTriggerTeamsSync,
  handleUpsertTeamsCredentials,
...
export function useTeamsConnector(projectId: string) {
  const [status, setStatus] = useState<TeamsIntegration | null>(null);
  const [credentials, setCredentials] = useState<TeamsCredentialStatus | null>(null);
  const [jobs, setJobs] = useState<TeamsSyncJob[]>([]);
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      const next = await handleGetTeamsStatus(projectId);
      setStatus(next);
      return next;
    } finally {
  ...
  const loadCredentials = useCallback(async () => {
    ...
    try {
      const next = await handleGetTeamsCredentialStatus(projectId);
      setCredentials(next);
      return next;
    } finally {
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      const next = await handleGetTeamsJobs(projectId);
      setJobs(next);
      return next;
    } finally {
  ...
  const onOAuthMessage = useCallback(
    (event: MessageEvent) => {
      if (event.data?.type === 'connector_connected' && event.data?.connector === 'teams') {
        void refetchAll();
      }
    },
    [refetchAll],
  ...
  const connect = useCallback(
    async (payload: TeamsConnectInput) => {
      if (!projectId) return;
      setIsConnecting(true);
      try {
        if (payload.save_credentials) {
          await handleUpsertTeamsCredentials({
        ...
        const authUrl = await handleGetTeamsAuthUrl(projectId);
        await openConnectorAuth(authUrl, payload.redirect_uri || getTeamsOAuthRedirectUri(), 'teams-oauth');
        if (Platform.OS !== 'web') {
          await refetchAll();
        }
      } finally {
  ...
  const saveSources = useCallback(
    async (selection: TeamsSourcesSelection) => {
      if (!projectId) throw new Error('errors.projectRequired');
      setIsSavingSources(true);
      try {
        const next = await handleSaveTeamsSources(projectId, selection);
        setStatus(next);
        return next;
      } finally {
  ...
  const saveSettings = useCallback(
    async (settings: TeamsIntegration['settings']) => {
      if (!projectId) throw new Error('errors.projectRequired');
      setIsSavingSettings(true);
      try {
        const next = await handleSaveTeamsSettings(projectId, settings);
        setStatus(next);
        return next;
      } finally {
  ...
  const triggerSync = useCallback(async () => {
    ...
    setIsSyncing(true);
    try {
      await handleTriggerTeamsSync(projectId);
  ...
  const pause = useCallback(async () => {
    ...
    try {
      const next = await handlePauseTeams(projectId);
      setStatus(next);
    } finally {
  ...
  const resume = useCallback(async () => {
    ...
    setActionPending(true);
    try {
      const next = await handleResumeTeams(projectId);
      setStatus(next);
    } finally {
      setActionPending(false);
    }
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      await handleDisconnectTeams(projectId);
      setStatus(null);
      setJobs([]);
    } finally {
